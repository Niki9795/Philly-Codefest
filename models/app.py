...
def main():
    ...
    # ===UI functions===
    def train_model_pipeline(audio_input, denoise, model_name, number_of_false, epoch):
        output = "Train Pipline Starting\nLoading..."
        yield output
        # Use the gr.Progress() to show the progress of training
        # Generate 100 unique random indices for the first axis
        indices = np.random.choice(
            X_false.shape[0], size=number_of_false, replace=False
        )

        # Select the elements using the generated indices
        selected_X_false = X_false[indices]

        speaker_auth_model = auth_model(
            clf=clf_pre, X_false=selected_X_false, batch_size=batch_size
        )
        output += "\nBase model loaded"
        yield output
        ...
        with gr.Tab("Enrollment"):
            ...
            with gr.Row():
                with gr.Group():
                    ...
                    number_of_false = gr.Number(label="Number of False", value=105)
                    demo_seg = gr.Slider(
                        5, 20, value=5, label="Segment", info="Choose between 5 and 20"
                    )
                    demo_epoch = gr.Number(label="Epoch", value=n_epochs)
                    demo_learning_rate = gr.Number(
            ...
            train_button.click(
                ...
                inputs=[audio_input, denoise, model_name, number_of_false, demo_epoch],
